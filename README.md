...
The ergonomic, `mpmath`/`Decimal`-flavoured user layer is exposed to Python
via pybind11. It is off by default so the core library still builds with no
external dependencies; enable it with `-DNAM_BUILD_PYTHON=ON` (pybind11 is
located via `find_package`, falling back to `FetchContent`):
